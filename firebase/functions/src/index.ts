 ...
 */
export const findNearestDriver = functions.firestore
  .document('rides/{rideId}')
  .onCreate(async (snap, context) => {
    ...
    driversSnapshot.forEach(doc => {
      if (declinedBy.includes(doc.id)) return; // Skip drivers who declined
 ...
 */
export const declineRide = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be logged in.');
  }
  ...
  driversSnapshot.forEach(doc => {
    if (declinedBy.includes(doc.id)) return;
